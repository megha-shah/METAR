...
from constants import CACHE_EXPIRE_TIME, BASE_URL, FILE_EXT
...
def fetch_weather_data(station_code):
    url = BASE_URL + station_code + FILE_EXT
    ...
    try:
        ...
        for value in second_row_list:
            if len(prev_value) >=3 and (prev_value[:3]=="SKC"
                                        or prev_value[:3]=="FEW"
                                        or prev_value[:3]=="SCT"
                                        or prev_value[:3]=="BKN"
                                        or prev_value[:3]=="OVC"):
            ...
            if "KT" in value:
                ...
                if (size == 5 and value[ :KT_index].isdigit()) or (size == 8
                                                                   and value[ :KT_index].isdigit()
                                                                   and value[5] == "G"
...
def set_wind(value):
    ...
    #conversion of velocity from knot to mph
